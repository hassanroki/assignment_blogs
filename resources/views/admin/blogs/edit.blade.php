...
@section('content')
    ...
    <form action="{{ route('blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $blog->title) }}" class="w-full border p-2 rounded">
            @error('title')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="short_desc" class="w-full border p-2 rounded">{{ old('short_desc', $blog->short_desc) }}</textarea>
            @error('short_desc')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="read_minutes" value="{{ old('read_minutes', $blog->read_minutes) }}"
                    min="1" max="60" class="w-full border p-2 rounded">
                @error('read_minutes')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <select name="isFeature" class="w-full border p-2 rounded">
                <option value="0" {{ old('isFeature', $blog->isFeature) == '0' ? 'selected' : '' }}>Normal</option>
                <option value="1" {{ old('isFeature', $blog->isFeature) == '1' ? 'selected' : '' }}>Featured</option>
            </select>
            @error('isFeature')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <textarea name="full_desc" rows="6" class="w-full border p-2 rounded">{{ old('full_desc', $blog->full_desc) }}</textarea>
            @error('full_desc')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
